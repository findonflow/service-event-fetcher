...
if [ -f "${MOD_FILE}" ]; then
	...
	go get "${PKG_NAME}@${VERSION}" || {
		echo "go get the package failed"
		exit 1
	}
	...
	{
		echo "couldn't find go.mod file - make sure the script is in the project root directory"
		exit 1
	}
fi
...
(
	cd "${PKG_DIR}" || {
		echo "cd into the GOPATH package folder failed"
		exit 1
	}
	go generate
)
